...
async function fetchPreviousSummary(period, currentDate = new Date()) {
  try {
    ...
    const snapshot = await db.collection(COLLECTIONS.INSIGHTS_SUMMARY).doc(docId).get();
...
export const onIncomeCreated = onDocumentCreated(`${COLLECTIONS.INCOME}/{id}`, async (event) => {
  await handleTransactionEvent(event);
});

export const onExpenseCreated = onDocumentCreated(`${COLLECTIONS.EXPENSE}/{id}`, async (event) => {
  await handleTransactionEvent(event);
});

export const onIncomeChanged = onDocumentWritten(`${COLLECTIONS.INCOME}/{id}`, async (event) => {
  await handleTransactionEvent(event);
});

export const onExpenseChanged = onDocumentWritten(`${COLLECTIONS.EXPENSE}/{id}`, async (event) => {
  await handleTransactionEvent(event);
});
...
export const manualInsightTrigger = onDocumentWritten(`${COLLECTIONS.TRIGGERS}/force-insight-generation`, async (event) => {
  ...
  const generatePeriodCounters = after.generatePeriodCounters && !before.generatePeriodCounters;
  ...
  if (generatePeriodCounters) {
    await generateAllPeriodCounters();
  }

  // Reset flags after execution
  await db.doc(`${COLLECTIONS.TRIGGERS}/force-insight-generation`).update({
    generateToday: false,
    generateAll: false,
    generatePeriodCounters: false
  });
});
